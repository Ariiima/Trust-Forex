...
```
Gateways config: server/gateways.json (manual admin file), per network: { network, address, memo?, tokenContract?, decimals, chain: 'evm'|'btc'|'tron'|'sol', rpc?, requiredConfirmations }. Admin verify override: Telegram bot inline Approve/Reject (long-polling, stdlib fetch, no SDK); on confirm (auto or manual) bot messages the paying user too. Env: TF_BOT_TOKEN, TF_ADMIN_CHAT_ID, TF_PORT, TF_DEV.
