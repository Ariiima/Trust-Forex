...
Many users pay the SAME wallet, so each active order gets a unique crypto amount: base = amountUsd/rate rounded UP per currency display decimals, then SUBTRACT a random dither in the last decimals (stablecoins: 2 decimals, 1–99 cents, e.g. 200 → 199.37; BTC/ETH/BNB/SOL/TRX/TON: last 3 of their 6-8 display decimals). Only an amount too small to subtract from (a sub-dollar balance remainder) dithers upward instead. Regenerate on collision with any non-final order of same (currency, network, address). amount_crypto stored as DECIMAL STRING, never float. A transfer matches an order iff to-address matches AND normalized on-chain amount === amount_crypto exactly AND tx time ≥ order created_at AND txid not in seen_txs. Memo (where the design shows one) is display-only extra identification, NOT required for matching (EVM/TRON/BTC wallets can't send memos).
